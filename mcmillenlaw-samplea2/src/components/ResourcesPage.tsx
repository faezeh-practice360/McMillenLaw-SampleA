          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
            {allArticles.slice(0, visibleCount).map((article) => (
              <div 
                ...
                className="bg-[#132444] text-white overflow-hidden border border-white/10 hover:border-[#D4A74A]/40 transition-all flex flex-col justify-between group h-full p-6 cursor-pointer"
              >
                <div className="space-y-4">
                ...
                <div className="pt-4 flex items-center justify-between text-xs text-white/60 border-t border-white/10 mt-4">
                  ...
                  <button 
                    ...
                    className="text-[#D4A74A] hover:text-white font-bold uppercase tracking-wider flex items-center gap-1 transition-colors bg-transparent border-none cursor-pointer text-[11px]"
                  >
                    {isEs ? "Leer" : "Read"} <ArrowRight size={13} />
                  </button>
                </div>
              </div>
            ))}
      ...
      {activeArticle && (
        ...
        >
          <div 
            ...
            className="bg-[#132444] text-white border border-[#D4A74A]/40 max-w-[700px] w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 shadow-2xl relative space-y-6"
          >
            <button
              onClick={() => setActiveArticle(null)}
              className="absolute top-4 right-4 text-white/60 hover:text-white bg-white/10 hover:bg-white/20 p-2 cursor-pointer transition-colors"
            >
              ✕
            </button>

            <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#D4A74A]">
              <span className="w-2 h-2 bg-[#D4A74A]" />
            ...
            <img
              ...
              className="w-full h-64 sm:h-80 object-cover border border-white/10"
            />
            ...
            <p className="text-white/80 text-sm leading-relaxed">
              {isEs 
                ? "En McMillen Legal, nuestro compromiso es brindar asesoría legal clara y sólida. Si tiene preguntas o necesita asistencia jurídica directa para su caso, comuníquese con nosotros."
                : "At McMillen Legal, our commitment is to provide clear and robust legal counsel. If you have questions or require direct legal assistance for your case, please reach out to our team."
              }
            </p>
            ...
            <div className="pt-4 border-t border-white/10 flex flex-col sm:flex-row gap-4">
              <button
                ...
                className="bg-[#D4A74A] hover:bg-white text-[#132444] px-6 py-3.5 text-xs font-bold uppercase tracking-widest transition-all cursor-pointer flex-1 text-center"
              >
                {isEs ? "PROGRAMAR CONSULTA" : "SCHEDULE A CONSULTATION"}
              </button>
              <a
                href="tel:9184190176"
                className="bg-white/10 hover:bg-white/20 text-white border border-white/20 px-6 py-3.5 text-xs font-bold uppercase tracking-widest transition-all text-center flex items-center justify-center gap-2"
              >
                <Phone size={14} className="text-[#D4A74A]" />
                918.419.0176
              </a>
            </div>
          </div>
